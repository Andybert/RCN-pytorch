...
class RNNCell(RNNCellBase):
    ...
    def forward(self, input, h):
        output = input + F.linear(F.dropout(torch.tanh(h), 0.6), self.weight_hh)
        if self.grad_clip:
...
class LSTMCell(RNNCellBase):
    ...
    def forward(self, input, hx):
        ...
        g = torch.tanh(pre[:, self.hidden_size * 2: self.hidden_size * 3])
...
class RNNBase(Module):
    ...
    def forward(self, input, initial_states=None):
        if initial_states is None:
            # zeros = Variable(torch.zeros(1, self.hidden_size)).cuda()
            zeros = torch.zeros((1, self.hidden_size),device=input.device)
            if self.mode == 'LSTM':
                initial_states = [(zeros, zeros), ] * self.num_layers
            elif self.mode == 'LSTMP':
                # zeros_h = Variable(torch.zeros(1, self.recurrent_size)).cuda()
                zeros_h = torch.zeros((1, self.recurrent_size),device=input.device)
